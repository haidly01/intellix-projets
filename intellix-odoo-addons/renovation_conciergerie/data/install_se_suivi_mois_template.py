...
NAME = "SE — Suivi à 1 mois"
XMLID = "renovation_conciergerie.mail_template_se_suivi_mois"
HTML_PATH = Path("/odoo/custom/addons/renovation_conciergerie/data/email_se_suivi_mois.html")
...
vals = {
    "name": NAME,
    "model_id": model.id,
    "subject": "Votre projet de {{ object.service_category_ids[:1].name if object.service_category_ids else 'rénovation' }} est-il toujours d'actualité ?",
    "body_html": BODY,
...
if not env.ref(XMLID, raise_if_not_found=False):
    env["ir.model.data"].sudo().create({
        "name": "mail_template_se_suivi_mois",
