...
def _build_env_map() -> dict[str, str]:
    ...
    return {
        ...
        "VICIDIAL_CAMPAIGN": "SE_RENOV_QC",
        "VICIDIAL_LIST": "SE_RENOV_QC",
        "CAMPAIGN_ID": "sofia-soumission-qc-2026",
        "N8N_BASE_URL": base,
        "N8N_WEBHOOK_URL": _read_conf("N8N_WEBHOOK_URL") or (base + "/webhook"),
        "ODOO_URL": _read_conf("ODOO_URL") or "https://intellixcrm.com",
        "STORAGE_BUCKET_URL": "https://intellixcrm.com/soumission-qc-tts",
...
def sync_n8n_env(env_map: dict[str, str]) -> None:
    if not N8N_ENV.is_file():
        ...
        return
    start, end = "# --- soumission-qc ---", "# --- end soumission-qc ---"
...
def main() -> None:
    ...
    print("Webhooks: /webhook/soumission-qc/{outbound,event,conversation,qualified,non-qualified,stats}")
