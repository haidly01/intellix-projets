...
if (provider === 'twilio') {
  ...
  await this.helpers.httpRequest({
    ...
    body: `To=${encodeURIComponent(phone)}&From=${encodeURIComponent(from)}&Url=${encodeURIComponent(base + '/lea-qc/twilio/voice')}&StatusCallback=${encodeURIComponent(base + '/lea-qc/event')}`,
  });
...
const res = await this.helpers.httpRequest({
  method: 'POST',
  url: odooUrl + '/doorway/api/lea-qc/dial',
  body: {
    jsonrpc: '2.0',
    method: 'call',
    params: {
