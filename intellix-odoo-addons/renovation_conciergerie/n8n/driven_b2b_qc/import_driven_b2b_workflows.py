#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Importe les workflows Alex Driven B2B QC dans n8n."""
...
def _build_env_map() -> dict[str, str]:
    ...
    return {
        ...
        "VICIDIAL_CAMPAIGN": "DW_QCB2B",
        "VICIDIAL_LIST": "DW_QCB2B",
        "CAMPAIGN_ID": "driven-b2b-qc-2026",
        "N8N_BASE_URL": base,
        "N8N_WEBHOOK_URL": _read_conf("N8N_WEBHOOK_URL") or (base + "/webhook"),
        "ODOO_URL": _read_conf("ODOO_URL") or "https://intellixcrm.com",
        "DRIVEN_STORAGE_BUCKET_URL": "https://intellixcrm.com/driven-b2b-tts",
        "DRIVEN_INTERNAL_WEBHOOK_URL": "http://127.0.0.1:5678/webhook",
        "AGENT_SOFT_LIMIT_SEC": "120",
        "DRIVEN_QC_RATE_PER_MIN": "0.25",
        ...
        "ELEVENLABS_VOICE_ID_ALEX": _read_conf("ELEVENLABS_VOICE_ID_ALEX") or _read_conf("ELEVENLABS_VOICE_ID"),
...
def sync_n8n_env(env_map: dict[str, str]) -> None:
    if not N8N_ENV.is_file():
        ...
        return
    start, end = "# --- driven-b2b-qc ---", "# --- end driven-b2b-qc ---"
...
def main() -> None:
    ...
    if not token:
        print("N8N_API_TOKEN required — use patch_driven_b2b_n8n_sqlite.py", file=sys.stderr)
        sys.exit(1)
    ...
    print("Webhooks: /webhook/driven-b2b-qc/{outbound,event,conversation,qualified,non-qualified,stats}")
