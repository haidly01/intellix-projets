...
class RenofacileFrApiController(http.Controller):
    ...
    def call_ended(self, tenant_api_key, **payload):
        ...
        from odoo.addons.doorway_agents_dashboard.services.renofacile_fr_service import (
            RenofacileFrService,
        )

        crm = RenofacileFrService(request.env(su=True)).process_call_ended(payload)
        return {"success": True, "crm": crm}

    @http.route(
        "/doorway/api/renofacile-fr/call-recording",
        type="jsonrpc",
        auth="public",
        methods=["POST"],
        csrf=False,
    ...
    def call_recording(self, tenant_api_key, call_sid, recording_url, **payload):
        """Enregistrement Sofia FR prêt — upsert doorway.call.log."""
        tenant = self._tenant_from_key(tenant_api_key)
        if not tenant:
            return {"success": False, "error": "invalid_api_key"}
        ...
        try:
            ...
            log_rec = VicidialService(request.env(su=True)).upsert_sofia_call_recording(
                {
                    **payload,
                    ...
                    "campaign": payload.get("campaign") or "DW_FRB2C",
                    "agent_id": payload.get("agent_id") or "marenofacile",
                },
            ...
            _logger.exception("RénoFacile call recording upsert failed: %s", exc)
