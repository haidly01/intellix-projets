...
class CoinsQuebecPartenariatBonus(models.Model):
    ...
    cq_rdv_followup_note = fields.Text(
        string="Note Martin (RDV)",
        tracking=True,
        help="Dernière note saisie par Martin sur le rendez-vous. Visible sur le kanban.",
    )
    ...
    entente_signee_paiement_recu = fields.Boolean(
        string="Entente signée et paiement reçu",
        default=False,
        tracking=True,
        help="Martin coche seulement si l'entente est signée et l'argent reçu. "
        "Pas une estimation. Sans cette case, le montant vendu ne compte "
        "pas pour le bonus revenu.",
    )
    ...
    def _compute_agent_metrics(self, agent, month_date):
        ...
        for fiche in fiches:
            ...
            if status == "present":
                ...
                absents += 1
            # Revenu bonus : montant seulement si entente signée ET paiement reçu.
            # Présence (1000 DH) reste indépendante de cette case.
            if fiche.entente_signee_paiement_recu:
